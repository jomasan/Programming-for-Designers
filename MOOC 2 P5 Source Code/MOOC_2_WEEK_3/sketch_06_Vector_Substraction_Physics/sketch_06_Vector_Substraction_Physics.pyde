...
def draw():
    ...
    #Vector multiplication scales the vector to our desired size, here it is 0.3
    vec_difference.mult(3)

    #add the velocity to the acceleration vector - the acceleration vector will not be reset from frame to frame
    vec_acceleration.add(vec_difference)

    #Let's limit the acceleration vector to a max velocity of 3 - this could also be a variable
    vec_acceleration.limit(3)
    
    #Vector addition
    #Add acceleration to velocity
    vec_velocity.add(vec_acceleration)

     #Let's also limit the velocity vector
    vec_velocity.limit(2)
