...
def setup():
    ...
    #create a font
    f = createFont('Corbel Bold', 24)
    textFont(f)
    textAlign(LEFT, CENTER)

    #use this method to see what fonts are offered by processing
    #print( PFont.list() )
...
def draw():
    ...
    if frameCount % 1 == 0:
        ...
        #define a how large the cell we will use for a grid location of the letter
        cell_x = image_size_x / 60
        cell_y = image_size_y / 30

        #now let's calculate a grid position based on the position of the mouse relative to the cell size we difined
        pos_x = round(mouseX / cell_x) * cell_x
        pos_y = round(mouseY / cell_y) * cell_y

        #define a constant size
        size = 20  

        #define a random character
        char = random.randrange(0,len(possible_characters))

        textSize(size)

        #display a text in the screen at the coordinate specified
        text( possible_characters[char], pos_x,pos_y)
